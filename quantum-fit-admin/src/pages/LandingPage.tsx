...
import type { LandingContent, Testimonial, Plan, GalleryImage, Gym } from '../types';
...
function SedesSection() {
  ...
  const [formData, setFormData] = useState<Partial<Gym>>({
    name: '', address: '', city: '', phone: '', hours: '', isActive: true,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await gymService.update(editing.id, formData);
      } else {
        await gymService.create(formData);
      }
      resetForm();
      loadItems();
    } catch (err: unknown) {
