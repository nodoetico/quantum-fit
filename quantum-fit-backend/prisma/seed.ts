...
async function main() {
  ...
  const achievements = [
    { id: 'ach_001', name: 'Primera Visita', description: 'Completa tu primera sesión en QUANTUM FIT', icon: '🎯', achievementType: 'first_checkin', requirementType: 'count', requirementValue: 1, pointsReward: 50, category: 'ASISTENCIA', displayOrder: 1 },
    { id: 'ach_002', name: 'Constante', description: 'Mantén una racha de 7 días consecutivos', icon: '🔥', achievementType: 'streak', requirementType: 'count', requirementValue: 7, pointsReward: 100, category: 'ASISTENCIA', displayOrder: 2 },
    { id: 'ach_003', name: 'Imparable', description: 'Mantén una racha de 30 días consecutivos', icon: '💥', achievementType: 'streak', requirementType: 'count', requirementValue: 30, pointsReward: 500, category: 'ASISTENCIA', displayOrder: 3 },
    { id: 'ach_004', name: 'Semana Perfecta', description: 'Asiste los 7 días de una semana', icon: '👑', achievementType: 'perfect_week', requirementType: 'count', requirementValue: 1, pointsReward: 200, category: 'ESPECIAL', displayOrder: 4 },
    { id: 'ach_005', name: 'Guerrero', description: 'Completa 25 entrenamientos', icon: '💪', achievementType: 'total_workouts', requirementType: 'count', requirementValue: 25, pointsReward: 200, category: 'RUTINA', displayOrder: 5 },
    { id: 'ach_006', name: 'Bestia', description: 'Completa 100 entrenamientos', icon: '🦍', achievementType: 'total_workouts', requirementType: 'count', requirementValue: 100, pointsReward: 500, category: 'RUTINA', displayOrder: 6 },
    { id: 'ach_007', name: 'Leyenda', description: 'Completa 500 entrenamientos', icon: '🐉', achievementType: 'total_workouts', requirementType: 'count', requirementValue: 500, pointsReward: 1000, category: 'RUTINA', displayOrder: 7 },
    { id: 'ach_008', name: 'Cazador de Puntos', description: 'Acumula 1000 puntos totales', icon: '💎', achievementType: 'points_earned', requirementType: 'count', requirementValue: 1000, pointsReward: 150, category: 'ACTIVIDAD', displayOrder: 8 },
    { id: 'ach_009', name: 'Nivel 5', description: 'Alcanza el nivel 5', icon: '⭐', achievementType: 'level_reached', requirementType: 'count', requirementValue: 5, pointsReward: 250, category: 'ASISTENCIA', displayOrder: 9 },
    { id: 'ach_010', name: 'Nivel 10', description: 'Alcanza el nivel 10', icon: '🌟', achievementType: 'level_reached', requirementType: 'count', requirementValue: 10, pointsReward: 500, category: 'ASISTENCIA', displayOrder: 10 },
  ];
  for (const achievement of achievements) {
    await prisma.achievement.create({ data: achievement }).catch(() => {});
  }
  achievementsCount = achievements.length;
  console.log(`✅ ${achievementsCount} logros creados`);
  } else {
  ...
  try {
    ...
    console.log('⚠️ Gym upsert skipped (schema drift)');
  }
  ...
  const rewards = [
    { id: 'reward_001', name: 'Proteína Whey', description: 'Batido de proteína post-entreno (30g)', pointsCost: 500, category: 'PRODUCTO', stockTotal: 50, stockAvailable: 50, isActive: true },
    { id: 'reward_002', name: 'Smoothie Energético', description: 'Smoothie natural de frutas y energía', pointsCost: 300, category: 'BEBIDA', stockTotal: 100, stockAvailable: 100, isActive: true },
    { id: 'reward_003', name: '10% Descuento Membresía', description: 'Descuento aplicable a tu próxima renovación', pointsCost: 2000, category: 'DESCUENTO', stockTotal: 20, stockAvailable: 20, isActive: true, isFeatured: true },
    { id: 'reward_004', name: 'Camiseta QUANTUM', description: 'Camiseta oficial del gimnasio', pointsCost: 1500, category: 'PRODUCTO', stockTotal: 30, stockAvailable: 30, isActive: true },
    { id: 'reward_005', name: 'Sesión con Entrenador', description: 'Sesión personalizada de 30 minutos', pointsCost: 1000, category: 'PROMOCION', stockTotal: 10, stockAvailable: 10, isActive: true },
    { id: 'reward_006', name: 'Barra Proteica', description: 'Barra energética alta en proteínas', pointsCost: 200, category: 'BEBIDA', stockTotal: 200, stockAvailable: 200, isActive: true },
    { id: 'reward_007', name: 'Toalla Premium', description: 'Toalla de microfibra QUANTUM FIT', pointsCost: 800, category: 'PRODUCTO', stockTotal: 25, stockAvailable: 25, isActive: true },
    { id: 'reward_008', name: 'Mes Libre de Guest Pass', description: 'Invita a un amigo gratis por un mes', pointsCost: 1200, category: 'PROMOCION', stockTotal: 15, stockAvailable: 15, isActive: true },
  ];
  for (const reward of rewards) {
    await prisma.reward.create({ data: reward }).catch(() => {});
  }
  rewardsCount = rewards.length;
  console.log(`✅ ${rewardsCount} recompensas creadas`);
  } else {
    console.log('📦 Recompensas ya existen — saltando');
  }
  ...
  // ============================================
  let landingContentCount = 0, testimonialCount = 0, planCount = 0, galleryCount = 0;
  if ((await prisma.landingContent.count()) === 0) {
    console.log('🌐 Creando contenido de Landing Page...');
    await prisma.landingContent.create({ data: { id: 'landing_hero_001', section: 'hero', title: 'Transformá tu cuerpo,<br />evolucioná tu mente', subtitle: 'Transformación física y mental', description: 'En Quantum Fit no es solo entrenar. Es descubrir tu mejor versión. Entrenamiento de élite, nutrición inteligente y una comunidad que te impulsa.', ctaText: 'Ver planes', ctaLink: '#planes', isActive: true, order: 0 } }).catch(() => {});
    await prisma.landingContent.create({ data: { id: 'landing_about_001', section: 'about', title: 'Excelencia en movimiento', description: 'En Quantum Fit combinamos entrenamiento de élite con un ambiente único. Instalaciones de primera, instructores certificados y una comunidad que te impulsa a dar lo mejor.', isActive: true, order: 0 } }).catch(() => {});
    landingContentCount = 2;
  }
  if ((await prisma.testimonial.count()) === 0) {
    ...
    for (const t of landingTestimonials) { await prisma.testimonial.create({ data: t }).catch(() => {}); }
    testimonialCount = landingTestimonials.length;
  }
  if ((await prisma.plan.count()) === 0) {
    const landingPlans = [
      { id: 'plan_001', name: 'Básico', price: 8000, period: 'mensual', currency: 'ARS', features: ['Acceso a una sede', 'Horario libre (8-22hs)', 'Evaluación física inicial', 'App de seguimiento'], isFeatured: false, order: 1 },
      { id: 'plan_002', name: 'Premium', price: 12000, period: 'mensual', currency: 'ARS', features: ['Acceso a todas las sedes', 'Horario extendido (6-23hs)', 'Clases grupales ilimitadas', 'Plan de nutrición personalizado', 'App de seguimiento'], isFeatured: true, order: 2 },
      { id: 'plan_003', name: 'VIP', price: 18000, period: 'mensual', currency: 'ARS', features: ['Acceso ilimitado a todas las sedes', 'Horario 24/7', 'Entrenador personal incluido', 'Clases grupales ilimitadas', 'Plan de nutrición personalizado', 'App de seguimiento premium'], isFeatured: false, order: 3 },
      { id: 'plan_004', name: 'Plan Anual', price: 42000, period: 'anual', currency: 'ARS', features: ['Todo del plan semestral', '6 meses gratis', 'Merchandising oficial', 'Invitado gratis cada mes'], isFeatured: false, order: 4 },
    ];
    for (const p of landingPlans) { await prisma.plan.create({ data: p }).catch(() => {}); }
    planCount = landingPlans.length;
  }
  if ((await prisma.galleryImage.count()) === 0) {
    ...
    for (const img of galleryImages) { await prisma.galleryImage.create({ data: img }).catch(() => {}); }
    galleryCount = galleryImages.length;
  }
  if (!(await prisma.gym.findUnique({ where: { id: 'gym_centro' } }))) {
    const landingGyms = [
      { id: 'gym_centro', name: 'Sede Centro', address: 'Av. Corrientes 1234, CABA', city: 'CABA', phone: '+54 11 1234-5678', hours: 'Lun a Vie: 6:00 - 23:00 | Sáb: 8:00 - 20:00', latitude: -34.6037, longitude: -58.3816, isActive: true },
      { id: 'gym_belgrano', name: 'Sede Belgrano', address: 'Av. Cabildo 2345, CABA', city: 'CABA', phone: '+54 11 2345-6789', hours: 'Lun a Vie: 6:00 - 23:00 | Sáb: 8:00 - 20:00', latitude: -34.5630, longitude: -58.4580, isActive: true },
    ];
    for (const lg of landingGyms) { await prisma.gym.create({ data: lg }).catch(() => {}); }
  }

  console.log('═══════════════════════════════════════════════════════');
  if (achievementsCount || landingContentCount || testimonialCount || planCount || galleryCount || rewardsCount) {
    let summaryParts: string[] = [];
    if (achievementsCount) summaryParts.push(`${achievementsCount} Logros`);
    if (rewardsCount) summaryParts.push(`${rewardsCount} Recompensas`);
    if (landingContentCount) summaryParts.push(`${landingContentCount} Contenidos Landing`);
    if (testimonialCount) summaryParts.push(`${testimonialCount} Testimonios`);
    if (planCount) summaryParts.push(`${planCount} Planes`);
    if (galleryCount) summaryParts.push(`${galleryCount} Imágenes de Galería`);
  }
  ...
  for (const c of courses) { await prisma.course.create({ data: c }).catch(() => {}); }
  ...
  // 9. Buffet Items
  // ============================================
  if ((await prisma.buffetItem.count()) === 0) {
  ...
  for (const item of buffetItems) { await prisma.buffetItem.create({ data: item }).catch(() => {}); }
  ...
  for (const n of newsItems) { await prisma.news.create({ data: n }).catch(() => {}); }
